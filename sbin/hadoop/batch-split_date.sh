...
home="data/dump/gollum01.1st"

for dir_name in $(ls -d ${home}/*/) ; do
    echo ${dir_name}
    db_name=$(basename ${dir_name})

    for filename in $(ls ${home}/${db_name}/*.json.bz2) ; do
        ...
        # 결과 폴더: 로컬 저장소
        result_dir="data/dump/by_date/${db_name}"
        if [ ! -d ${result_dir} ] ; then
            mkdir -p "${result_dir}"
        fi

        # 입력 파일 업로드 폴더: 하둡 저장소
        corpus_dir="/user/ejpark/corpus/${db_name}"
