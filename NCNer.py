...
from NCApiCenter import NCApiCenter
...
class NCNer:
    ...
    def read_corpus(self, host_name, port, db_name, collection_name, sample_size=-1):
        ...
        """
        from NCMongoDB import NCMongoDB

        mongo_db = NCMongoDB()
        mongo_db.open(db_name=db_name, host_name=host_name, port=port)

        cursor = mongo_db.db[collection_name].find({})
    ...
    # def read_corpus(self, filename_corpus, sample_size=-1):
    #     """
    #     학습셋 말뭉치 디비에서 학습셋을 읽음.
    #     """
    #     import sqlite3
    #
    #     connect = sqlite3.connect(filename_corpus)
    #     cursor = connect.cursor()
    #
    #     if sample_size < 0:
    #         sql = "SELECT ne_tagged FROM ne_tagged ORDER BY RANDOM()"
    #     else:
    #         sql = "SELECT ne_tagged FROM ne_tagged ORDER BY RANDOM() LIMIT {}".format(sample_size)
    #         # sql = "SELECT ne_tagged FROM ne_tagged WHERE sentence LIKE '%루크 스캇%' LIMIT 1"
    #
    #     corpus = []
    #     for row in cursor.execute(sql):
    #         sentence_token = self.split_ne_tag(row[0])
    #         sentence_token = self.bi_tagging(sentence_token)
    #
...
if __name__ == '__main__':
    ne_tagger = NCNer()
    ne_tagger.parse_argument()

    if ne_tagger.args.cross_evaluation is True:
        ne_tagger.cross_evaluation()
    elif ne_tagger.args.train_single_model is True:
        ne_tagger.train_single_model()
    elif ne_tagger.args.eval is True:
        eval_sentences = []
        for line in sys.stdin:
            eval_sentences.append(ne_tagger.split_ne_tag(line.strip()))

        ne_tagger.evaluate_model(ne_tagger.args.model, eval_sentences)
    elif ne_tagger.args.get_features is True:
        # 스텐포드 NER 학습용 자질 추출
        eval_sentences = []
        for line in sys.stdin:
    ...
    elif ne_tagger.args.stdin is True:
        ne_tagger.open(ne_tagger.args.model)
