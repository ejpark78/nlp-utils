...
class SeleniumCrawler(object):
    ...
    def rename_doc_id(self):
        ...
        while start < len(id_list):
            ...
            for doc in tqdm(doc_list, desc='{:,}~{:,}'.format(start, end)):
                url = self.parse_url(url=doc['url'])
                # doc['_id'] = '{clubid}-{articleid}'.format(**url)
    ...
    def trace_contents(self, bbs_info):
        ...
        doc_list = []
        self.elastic.get_by_ids(id_list=list(id_list), index=self.elastic.index, source=None, result=doc_list)
