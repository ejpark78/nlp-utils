...
class CacheUtils(CacheBase):
    ...
    def save_account(self, document: dict, name: str, account_id: str) -> None:
        self.cursor.execute(self.template['accounts'], (account_id, name, json.dumps(document, ensure_ascii=False),))
        self.conn.commit()
        return

    def save_post_count(self, account_id: str, count: int) -> None:
        self.cursor.execute(self.template['post_count'], (count, account_id), )
        self.conn.commit()
        return

    def save_post(self, document: dict, post_id: str, account_id: str, date: str = None) -> None:
        if date is None:
    ...
    def save_replies(self, document: dict, post_id: str, reply_id: str, date: str = None) -> None:
        if date is None:
