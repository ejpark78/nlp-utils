...
class NLUWrapper(object):
    ...
    def request(self, doc_list: list, options: dict, url: str, timeout: int, domain: str, style: str) -> list:
        if len(doc_list) == 0:
            return []
    ...
    def make_sql_frame(self, table_name: str) -> None:
        ...
        self.result_db['column_keys'] = list(set(self.result_db['alias'].keys()))

        str_col = '`' + '`,`'.join([self.result_db['alias'][x] for x in self.result_db['column_keys']]) + '`'
        str_val = ','.join(['%s'] * len(self.result_db['column_keys']))

        self.result_db['sql'] = f'REPLACE INTO `{table_name}` ({str_col}) VALUES ({str_val})'

        return

    def get_values(self, doc: dict) -> list:
        result = []
        for col in self.result_db['column_keys']:
            if col not in doc:
                result.append('')
                continue
    ...
    def save_result(self, doc_list: list, db: pymysql.connections.Connection) -> None:
        ...
        for doc in doc_list:
            ...
            try:
                cursor.execute(self.result_db['sql'], self.get_values(doc=doc))
            except Exception as e:
    ...
    @staticmethod
    def save_error_list(error_list: set, filename: str = 'nlu-wrapper-error.list') -> None:
    ...
    def batch(self) -> None:
        ...
        self.make_sql_frame(table_name=self.params['result_table_name'])
        ...
        for doc in tqdm(doc_list, desc='NLU Wrapper'):
            bulk += self.make_request(
                document=doc,
                modules=set(self.nlu_wrapper_options['module'])
            )

            if len(bulk) > self.params['bulk_size']:
                resp = self.request(
                    url=self.params['nlu_wrapper_url'],
                    doc_list=bulk,
                    style='literary',
                    domain='economy',
                    options={**self.nlu_wrapper_options},
                    timeout=self.params['timeout']
                )
                self.save_result(doc_list=resp, db=result_db)

                if len(resp) == 0:
                    error_list.update(set([(x['meta']['_index'], x['meta']['_id']) for x in bulk if 'meta' in x]))

                    self.save_error_list(error_list=error_list)
                    error_list = set()
