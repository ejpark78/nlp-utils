...
class NaverKBOGameCenterUtils(object):
    ...
    def batch(self):
        ...
        if env.game_info:
            self.get_game_info(date_range=env.date_range)

        if env.trace_comments:
            self.trace_comments(date_range=env.date_range)

        if env.set_state:
            self.set_state(game_id=env.game_id, state=env.state)
            return
