...
class LineageMConverter:
    ...
    def make_index(self):
        ...
        conn, cursor = self.open_sqlite('lineagem.sqlite3')
        ...
        for line in sys.stdin:
            ...
            if document_id.find('comment') < 0:
                ...
                simple = self.simplify(title, document)
                if simple is None:
                    continue
            else:
                ...
                for comment in document['commentList']:
                    simple = self.simplify(comment_title, comment)
                    if simple is None:
                        continue
...
def main():
    ...
    if args.to_xlsx is True:
        ...
        converter.make_index()
    elif args.merge_comment is True:
        converter.merge_comment()
    else:
        converter.merge_comment()
