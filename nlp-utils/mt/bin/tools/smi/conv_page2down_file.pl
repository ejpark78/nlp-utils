...
main: {
	...
	foreach my $fn ( @ARGV ) {
		...
		$wr = sprintf("http://cineaste.co.kr/bbs/%s", $wr);
		...
		next if( -e "page/$wr_id.html" );
		
		printf "# %s #######################################################\n", $wr_id;
		printf "wget ";
		printf "--user-agent=\"%s\" ", "Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1; SV1)";
		printf "--cookies=on --keep-session-cookies --save-cookies=page/%s.cookie ", $wr_id;
		printf "\"%s\" ", $wr;
		printf "-O \"page/%s.html\"\n", $wr_id;

		printf "mkdir -p file/%s; sync\n", $wr_id;
		...
		foreach my $line ( @url_list ) {
			...
			printf "wget ";
			printf "--user-agent=\"%s\" ", "Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1; SV1)";
			printf "--cookies=on --keep-session-cookies --load-cookies=page/%s.cookie ", $wr_id;
			printf "--referer=\"%s\" \"%s\" ", $wr, $url;
			printf "-O \"file/%s/%s\"\n", $wr_id, $smi;
			printf "sleep %f\n\n", 2+rand(5);
		}
