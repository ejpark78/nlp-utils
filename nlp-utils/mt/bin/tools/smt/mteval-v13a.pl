...
# print "# eval_docs\n";
# print Dumper %eval_docs;
# print "# tst_data\n";
# print Dumper %tst_data;
# print "# ref_data\n";
...
# ejpark added
# $overallScore->{ $sys }{ 'documents' }{ $doc }{ 'segments' }{ $seg }{ 'score' }
if( $detail == -1 ) {
	# print Dumper %BLEUOverall;

	my (%refs) = ();
	foreach my $sys ( keys %ref_data ) {
		foreach my $doc ( keys %{$ref_data{$sys}} ) {
			foreach my $seg ( sort {$a<=>$b} keys %{$ref_data{$sys}{$doc}{'SEGS'}} ) {
				push(@{$refs{$doc}{$seg}}, $ref_data{$sys}{$doc}{'SEGS'}{$seg});
			}
		}
	}

	foreach my $sys ( keys %BLEUOverall ) {
		foreach my $doc ( keys %{$BLEUOverall{$sys}{'documents'}} ) {
			foreach my $seg ( sort {$a<=>$b} keys %{$BLEUOverall{$sys}{'documents'}{$doc}{'segments'}} ) {
				printf "# Sentence BLEU: $seg\t%.4f\t%.4f\t%s\t%s\t%s\n", 
					$BLEUOverall{$sys}{'documents'}{$doc}{'segments'}{$seg}{'score'},
					$NISTOverall{$sys}{'documents'}{$doc}{'segments'}{$seg}{'score'},
					$eval_docs{$doc}{'SEGS'}{$seg},
					$tst_data{$sys}{$doc}{'SEGS'}{$seg},
					join("\t", @{$refs{$doc}{$seg}});
			}
		}
	}
...
#score each segment
	foreach my $seg ( sort{ $a <=> $b } keys( %{$tst_data{$sys}{$doc}{SEGS}} ) )
	{
		...
		# print "seg: $seg\n";
		# print Dumper $match_cnt;
		# print Dumper $tst_cnt;
		# print Dumper $ref_cnt;
		# print Dumper $tst_info;
		# print Dumper $ref_info;
		...
		if ( $method eq "BLEU" )
		{
			...
			# printf "Sentence BLEU:\t$seg\t%.4f\n", $segScore if( $detail == -1 );
		}
		if ( $method eq "NIST" )
		{
			my %DOCmt = ();
	...
	# print "cum_match\n";
	# print Dumper @cum_match;
	# print Dumper @cum_tst_cnt;
	# print Dumper @cum_ref_cnt;
	# print Dumper @cum_tst_info;
	# print Dumper @cum_ref_info;
...
sub bleu_score_nosmoothing
{
	...
	# print "# bleu_score_nosmoothing\n";
	# print Dumper $matching_ngrams;
	# print Dumper $tst_ngrams;
	# print Dumper $SCOREmt;
	...
	for ( my $j = 1; $j <= $max_Ngram; ++$j )
	{
		...
		else
		{
			...
			# print "$j: $matching_ngrams->[ $j ] / $tst_ngrams->[ $j ]\n";
		}
	}
