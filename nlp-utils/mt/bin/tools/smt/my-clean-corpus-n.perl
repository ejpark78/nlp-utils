...
print STDERR "clean-corpus.perl: processing $corpus1 & $corpus2 to $out1, $out2, cutoff $min-$max\n";
...
open(FO,">$out1") or die "Can't write $out1";
open(EO,">$out2") or die "Can't write $out2";
...
while(my $f = <F>) {
  ...
  my $e = <E>;
  die "$corpus2 is too short!" if !defined $e;
  chomp($e);
  chomp($f);
  if ($innr == 1) {
    $factored_flag = ($e =~ /\|/ || $f =~ /\|/);
  ...
  die "There is a blank factor in $corpus1 on line $innr: $f"
    if $f =~ /[ \|]\|/;
  die "There is a blank factor in $corpus2 on line $innr: $e"
    if $e =~ /[ \|]\|/;
...
print STDERR "\n";
my $e = <E>;
die "$corpus2 is too long!" if defined $e;
